...
const PostStats = ({ post }: PostProps) => {
  ...
  useEffect(() => {
    ...
    const getPostLikes = async () => {
      let res = await getLikes(
        location.pathname === "/saved" ? post.postId : post._id
      );
    ...
    const check = async () => {
      let res = await checkSavedPost({
        postId: location.pathname === "/saved" ? post.postId : post._id,
  ...
  const handleLikePost = async (e: React.MouseEvent) => {
    ...
    if (hasLiked) {
      ...
      likesArr = likesArr.filter(
        (user: string) => user !== currentUser?.user.username
      );
      setLikeCount(likesArr.length);
      let data = { likesArr: likesArr, id: post._id };
      updateLikes(data);
    } else {
      setIsLiked(true);
