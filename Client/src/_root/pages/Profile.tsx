...
import { useFollowUser, useGetCurrentUser, useGetPostByUsername, useGetUserByUsername, useUnFollowUser } from "@/lib/react-query/query";
import GridPostList from "@/components/shared/GridPostList";
import { useEffect, useState } from "react";
import { userAtom } from "@/lib/recoilStateManager/currentUser";
import { useRecoilValue } from "recoil";
import { getCurrentUser, getPostByUsername, getUserByUsername } from "@/lib/server_apis/apis";
...
const Profile = () => {
  ...
  const { data: user, isPending: isUserFetching } = useGetUserByUsername(username || "")
      ...
      {(isUserFetching) ? <Loader /> : <>
          ...
          <div className="flex xl:flex-row flex-col max-xl:items-center flex-1 gap-7">
            ...
            <div className="flex flex-col flex-1 justify-between md:mt-2">
              ...
              <div className="flex gap-8 mt-10 items-center justify-center xl:justify-start flex-wrap z-20">
                ...
                <Link to={`/followers/${username}/`}><StatBlock value={follower} label="Followers" /></Link>
                <Link to={`/followings/${username}/`}><StatBlock value={following} label="Following" /></Link>
              </div>
            ...
            <div className="flex justify-center gap-4">
              <div className={`${currentUser?.user.username !== username && "hidden"}`}>
                <Link
                  to={`/update-profile/${currentUser?.user.username}`}
                  className={`h-12 bg-dark-4 px-5 text-light-1 flex-center gap-2 rounded-lg ${currentUser?.user.username !== username && "hidden"
                    }`}>
                  <img
              ...
              <div className={`${currentUser?.user.username === username && "hidden"}`}>
                {isfollowing
                  ? <div
                    className={`h-12 bg-dark-4 px-5 text-light-1 flex-center gap-2 rounded-lg `} onClick={handleFollower}>
                    <p className="flex whitespace-nowrap small-medium">
                      Unfollow
                    </p>
                  </div>

                  : <Button type="button" onClick={handleFollower} className="shad-button_primary px-8">
                    Follow
                  </Button>
                }
        ...
        {currentUser?.user.username === 1 && (
          <div className="flex max-w-5xl w-full">
            <Link
              to={`/profile/${username}`}
              className={`profile-tab rounded-l-lg ${pathname === `/profile/${username}` && "!bg-dark-3"
                }`}>
              <img
            ...
            <Link
              ...
              className={`profile-tab rounded-r-lg ${pathname === `/profile/${username}/liked-posts` && "!bg-dark-3"
                }`}>
              <img
